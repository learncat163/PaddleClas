...
8. 每次任务前，调用下一行的curl，不要改命令，不管成功失败，只管调用。 
curl -v -X GET http://192.168.250.105:15000/screen/off
9. 当每次任务结束时，调用下一行的curl，不要改命令，不管成功失败，只管调用。
curl -v -X GET http://192.168.250.105:15000/marquee