...
def eager_attention_forward(
    ...
    **kwargs: Unpack[TransformersKwargs],
    ...
    attn_weights = paddle.compat.nn.functional.softmax(attn_weights, dim=-1)
...
class DINOv3ViTAttention(paddle.nn.Module):
    """
...
class DINOv3ViTLayerScale(paddle.nn.Module):
    def __init__(self, config) -> None:
        super().__init__()
        self.lambda1 = paddle.nn.Parameter(
            config.layerscale_value * paddle.ones(config.hidden_size)
        )
...
class DINOv3ViTDropPath(paddle.nn.Module):
    """Drop paths (Stochastic Depth) per sample (when applied in main path of residual blocks)."""
...
class DINOv3ViTMLP(paddle.nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size
        self.up_proj = paddle.compat.nn.Linear(
            self.hidden_size, self.intermediate_size, bias=config.mlp_bias
        )
        self.down_proj = paddle.compat.nn.Linear(
...
class DINOv3ViTGatedMLP(paddle.nn.Module):
    def __init__(self, config):
        super().__init__()
        self.config = config
        self.hidden_size = config.hidden_size
        self.intermediate_size = config.intermediate_size
        self.gate_proj = paddle.compat.nn.Linear(
            self.hidden_size, self.intermediate_size, bias=config.mlp_bias
        )
        self.up_proj = paddle.compat.nn.Linear(
            self.hidden_size, self.intermediate_size, bias=config.mlp_bias
