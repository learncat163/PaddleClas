...
ORIGINAL_TO_CONVERTED_KEY_MAPPING = {
    ...
    "patch_embed.proj": "embeddings.patch_embeddings",
    "periods": "inv_freq",
    "rope_embed": "rope_embeddings",
    "blocks.(\\d+).attn.proj": "layer.\\1.attention.o_proj",
    "blocks.(\\d+).attn.": "layer.\\1.attention.",
    "blocks.(\\d+).ls(\\d+).gamma": "layer.\\1.layer_scale\\2.lambda1",
    "blocks.(\\d+).mlp.fc1": "layer.\\1.mlp.up_proj",
    "blocks.(\\d+).mlp.fc2": "layer.\\1.mlp.down_proj",
    "blocks.(\\d+).mlp": "layer.\\1.mlp",
    "blocks.(\\d+).norm": "layer.\\1.norm",
    "w1": "gate_proj",
    "w2": "up_proj",
    "w3": "down_proj",
}
...
def split_qkv(state_dict: dict):
    ...
    for key in keys:
        ...
        q, k, v = paddle.chunk(qkv, 3, dim=0)
...
>>>>>>def get_dinov3_config(model_name: str) -> transformers.DINOv3ViTConfig:
    if model_name == "vits16_lvd1689m":
>>>>>>        return transformers.DINOv3ViTConfig(
            patch_size=16,
            hidden_size=384,
...
>>>>>>    resize = torchvision.transforms.Resize((resize_size, resize_size), antialias=True)
    ...
    return paddle.vision.transforms.Compose([to_tensor, resize, normalize])


def get_image_processor(resize_size: int = 224):
>>>>>>    return transformers.DINOv3ViTImageProcessorFast(
        do_resize=True, size={"height": resize_size, "width": resize_size}, resample=2
    )
...
def convert_and_test_dinov3_checkpoint(args):
    expected_outputs = {
        ...
        "vitb16_lvd1689m_patch": [-0.082523, -0.456272, -0.728029, -0.43068, -0.15288],
        "vitl16_lvd1689m_cls": [0.484527, -0.582214, 0.480636, 0.59204, 0.945166],
...
>>>>>>    model = transformers.DINOv3ViTModel(config).eval()
...
>>>>>>    original_state_dict = torch.load(state_dict_path, mmap=True)
    ...
    paddle.testing.assert_close(
        original_pixel_values, inputs["pixel_values"], atol=1e-06, rtol=1e-06
    )
    print("Preprocessing looks ok!")
    with paddle.no_grad(), paddle.autocast("cuda", dtype=paddle.float32):
    ...
    paddle.testing.assert_close(
        paddle.Tensor(actual_outputs[f"{model_name}_cls"]),
        paddle.Tensor(expected_outputs[f"{model_name}_cls"]),
        atol=0.001,
        rtol=0.001,
    )
    paddle.testing.assert_close(
        paddle.Tensor(actual_outputs[f"{model_name}_patch"]),
        paddle.Tensor(expected_outputs[f"{model_name}_patch"]),
        atol=0.001,
        rtol=0.001,
    )
